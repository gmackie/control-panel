        ...
        try {
          ...
          if (repo) {
            ...
            try {
              ...
              if (workflows && workflows.length > 0 && latestWorkflow) {
                dashboardApp.cicd = {
                  provider: "gitea-actions",
                  status: latestWorkflow.status === "completed" 
                    ? (latestWorkflow.conclusion === "success" ? "success" : "failed")
                    : latestWorkflow.status === "in_progress" ? "running" : "pending",
