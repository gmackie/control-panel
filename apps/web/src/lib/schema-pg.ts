...
// ===========================================
// Application Repositories - Link multiple repos to an app
// ===========================================
export const applicationRepositories = pgTable("application_repositories", {
  id: uuid("id").primaryKey().defaultRandom(),
  applicationId: uuid("application_id").notNull().references(() => applications.id, { onDelete: "cascade" }),
  
  // Repository provider
  provider: text("provider").notNull(), // gitea, github, gitlab, bitbucket
  
  // Repository info
  repoId: text("repo_id"), // Provider-specific ID
  ...
  role: text("role").notNull().default("primary"), // primary, mirror, archive
  syncEnabled: boolean("sync_enabled").default(true),
  
  // Sync status
  lastSyncAt: timestamp("last_sync_at"),
  syncStatus: text("sync_status").default("unknown"), // synced, ahead, behind, diverged, unknown
  
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
}, (table) => ({
  appIdIdx: index("app_repos_app_id_idx").on(table.applicationId),
  providerIdx: index("app_repos_provider_idx").on(table.provider),
  fullNameIdx: index("app_repos_full_name_idx").on(table.fullName),
  uniqueAppRepo: uniqueIndex("app_repos_unique_idx").on(table.applicationId, table.provider, table.fullName),
}));

// ===========================================
// Application Deployments - Link to K8s/Vercel/etc deployments
// ===========================================
export const applicationDeployments = pgTable("application_deployments", {
  ...
  // Deployment target
  provider: text("provider").notNull(), // kubernetes, vercel, railway, fly
  ...
  // Container registry
  registryImage: text("registry_image"), // e.g., registry.gmac.io/library/app
  ...
  appIdIdx: index("app_deploys_app_id_idx").on(table.applicationId),
  providerIdx: index("app_deploys_provider_idx").on(table.provider),
  envIdx: index("app_deploys_env_idx").on(table.environment),
}));
...
export const applicationsRelations = relations(applications, ({ many }) => ({
  ...
  repositories: many(applicationRepositories),
  linkedDeployments: many(applicationDeployments),
...
export const applicationRepositoriesRelations = relations(applicationRepositories, ({ one }) => ({
  application: one(applications, {
    fields: [applicationRepositories.applicationId],
    references: [applications.id],
  }),
}));

export const applicationDeploymentsRelations = relations(applicationDeployments, ({ one }) => ({
  application: one(applications, {
