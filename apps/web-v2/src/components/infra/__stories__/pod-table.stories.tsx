...
import { mockNodes, mockPods } from "@/__mocks__/fixtures";
import type { MultiClusterNode, MultiClusterPod } from "@/types/k8s";

/** Static presentational version of the Pod Table for Storybook */
function PodTableStatic({
  nodes,
  pods,
}: {
            ...
            <tbody>
              {nodes.map((node) => {
                ...
                const podCount = pods.filter(
                  (p) => p.nodeName === node.name && p.clusterId === node.clusterId
                ).length;
...
const meta: Meta<typeof PodTableStatic> = {
  title: "Components/PodTable",
  component: PodTableStatic,
};

export default meta;
type Story = StoryObj<typeof PodTableStatic>;

export const Default: Story = {
  args: { nodes: mockNodes, pods: mockPods },
};

export const ProductionOnly: Story = {
  args: {
    nodes: mockNodes.filter((n) => n.clusterId === "production"),
    pods: mockPods.filter((p) => p.clusterId === "production"),
  },
};
